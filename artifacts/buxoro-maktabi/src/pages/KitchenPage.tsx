...
function StaffCard({ s }: { s: typeof staff[0] }) {
  return (
    <div className="w-[75vw] sm:w-[55vw] md:w-[300px] mx-auto">
      <div className="relative rounded-2xl bg-white/[0.05] border border-white/[0.08] backdrop-blur-sm md:backdrop-blur-xl overflow-hidden">
        <div className="relative overflow-hidden">
          <div className={`absolute inset-0 bg-gradient-to-br ${s.grad} opacity-[0.12]`} />
          <div className="aspect-[4/5] bg-gradient-to-br from-gray-800 to-black flex items-center justify-center">
            <span className="text-5xl md:text-6xl font-bold text-white/15">{s.name.charAt(0)}</span>
            <div className="absolute bottom-0 left-0 right-0 h-1/2 bg-gradient-to-t from-background/95 to-transparent" />
          </div>
        </div>
        <div className="p-4 md:p-5">
          <h3 className="text-white font-bold text-sm md:text-base mb-0.5">{s.name}</h3>
          <span className={`inline-block text-[10px] font-semibold uppercase tracking-wider mb-2 bg-clip-text text-transparent bg-gradient-to-r ${s.grad}`}>{s.role}</span>
          <p className="text-white/50 text-[11px] md:text-xs leading-relaxed">{s.desc}</p>
        </div>
      </div>
    </div>
  );
...
function MealCard({ m }: { m: typeof meals[0] }) {
  return (
    <div className="w-[75vw] sm:w-[55vw] md:w-[340px] mx-auto">
      <div className="relative rounded-[1.6rem] bg-white/[0.05] border border-white/[0.08] backdrop-blur-sm md:backdrop-blur-xl overflow-hidden">
        ...
        <div className="p-4 md:p-5">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-white font-bold text-base md:text-lg">{m.name}</h3>
            <span className="text-primary text-[11px] font-semibold">{m.time}</span>
          </div>
          <ul className="space-y-1.5">
            {m.items.map((item, i) => (
              <li key={i} className="flex items-center gap-2 text-white/50 text-xs md:text-sm">
